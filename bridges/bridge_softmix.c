...
#include "../codecs/ex_opus.h" //Needed to check if participants, which use opus, have stereo enabled
#include <fftw3.h>
...
/*! Setting the sample rate tp 48000 by default if binaural is activated. */
#define SOFTMIX_MIN_BINAURAL_SAMPLE_RATE 48000	

/*! \brief Interval at which mixing will take place. Valid options are only 20 ms at the moment. */
...
#define CONVOLVE_PREALLOC 3
/*! Max size of the convolve buffer. */
#define CONVOLVE_MAX_BUFFER 4096
/*! The default sample size in an binaural enviroment with opus. */
...
struct softmix_bridge_data {
	...
	pthread_t thread;
	/*! The internal sampling rate of the bridge */
	unsigned int internal_rate;
	/*! The internal mixing interval (typicaly 20 ms) */
...
static void random_binaural_pos_change(struct softmix_bridge_data *softmix_data) 
{
	...
	if (softmix_data->convolve.chan_size > 1) {
		...
		for (i = softmix_data->convolve.chan_size - 1; i > 0; i--) {
			j = rand() % (i + 1);
			struct convolve_channel_pair *tmp = softmix_data->convolve.cchan_pair[i];
...
static float *get_hrir(int chan_pos, int position) {
	//Work arround for current HRIR which may be broken 
	if (chan_pos % 2)
...
static void init_convolve_channel(struct convolve_channel *channel, int hrtf_len, unsigned int chan_pos, unsigned int chan_side, unsigned int default_sample_size)
{
	...
	float *hrir = get_hrir(chan_pos, chan_side);
...
static void init_convolve_channel_pair(struct convolve_channel_pair *cchan_pair, int hrtf_len, unsigned int chan_pos, unsigned int default_sample_size) 
{
	...
	init_convolve_channel(&cchan_pair->chan_left, hrtf_len, chan_pos, LEFT, default_sample_size);
	init_convolve_channel(&cchan_pair->chan_right, hrtf_len, chan_pos, RIGHT, default_sample_size);
}

static void init_convolve_data(struct convolve_data *data, unsigned int default_sample_size) 
{
	data->pos_ids = calloc(sizeof(int), sizeof(int) * CONVOLVE_PREALLOC);
	data->chan_size = CONVOLVE_PREALLOC;
	data->number_channels = 0;
	data->cchan_pair = malloc(sizeof(struct convolve_channel_pair *) * CONVOLVE_PREALLOC);
	unsigned int i;
	for (i = 0; i < CONVOLVE_PREALLOC; i++)
...
static int set_binaural_data_join(struct convolve_data *data, unsigned int default_sample_size) 
{
	...
	if (data->chan_size < data->number_channels)  {
		//We realloc by one cause fftw init costs alot atm 
		data->chan_size += 1;
		int *tmp = realloc(data->pos_ids, data->chan_size * sizeof(int));
		if (tmp)
			data->pos_ids = tmp;
		else 
			ast_log(LOG_ERROR, "Realloc of binaural channels failed, probably runing out of memory...");
		data->pos_ids[data->chan_size - 1] = 0;
		struct convolve_channel_pair **tmp2 = realloc(data->cchan_pair, data->chan_size * sizeof(struct convolve_channel_pair *));
		if (tmp2)
...
{
	if (interval != 20) {
		/* We only accept 20 ms at the moment */
		interval = 20;
	}
...
static int softmix_bridge_join(struct ast_bridge *bridge, struct ast_bridge_channel *bridge_channel)
{
	...
	if (bridge->softmix.binaural_active.active) {
		if (strncmp(ast_channel_name(bridge_channel->chan), "CBAnn", 5 * sizeof(char)) != 0) {
			if (strcmp("opus", ast_format_get_name(bridge_channel->write_format)) == 0) {
				struct opus_attr *attr = ast_format_get_attribute_data(bridge_channel->write_format);
				if (attr != NULL) {
					if (attr->stereo == 1) {
						set_binaural = 1;
						softmix_data->internal_rate = attr->maxplayrate;
					} 
				}
			}
			skip_binaural = 0;
...
static int softmix_mixing_loop(struct ast_bridge *bridge)
{
	...
	while (!softmix_data->stop && bridge->num_active) {
		...
		if (update_all_rates) {
		/*	
			if (min_rate > softmix_data->internal_rate) 
				softmix_translate_helper_change_rate(&trans_helper, min_rate);
			else
		*/
		...
		if (bridge->softmix.binaural_active.active && softmix_data->convolve.binaural_active) {
			...
			for (idx = 0; idx < mixing_array.used_entries; idx++) {
				if (mixing_array.chan_pairs[idx] == NULL) {
					...
					k = 0;
					for (x = 0; x < softmix_data->default_sample_size; x++) {
						if (mixing_array.chan_pairs[idx]->chan_left.out_data[x] > 32767)
							ast_log(LOG_ERROR, "FRANK: clipping\n");
						if (mixing_array.chan_pairs[idx]->chan_left.out_data[x]< -32767)
							ast_log(LOG_ERROR, "FRANK: clipping negative\n");
		...
		AST_LIST_TRAVERSE(&bridge->channels, bridge_channel, entry) {
			...
			if (bridge->softmix.binaural_active.active && softmix_data->convolve.binaural_active && sc->binaural && !bridge_channel->binaural_suspended) {
			...
			} else if (bridge->softmix.binaural_active.active && softmix_data->convolve.binaural_active && sc->binaural && bridge_channel->binaural_suspended) {
				...
				sc->write_frame.datalen = softmix_datalen * 2;
				sc->write_frame.samples = softmix_samples * 2;
				memcpy(sc->final_buf, susp_buf, softmix_datalen * 2);
			} else {
...
static void *softmix_mixing_thread(void *data)
{
	...
	if (bridge->softmix.binaural_active.active) {
		softmix_data->internal_rate = SOFTMIX_MIN_BINAURAL_SAMPLE_RATE;
